...
tidy_res <- broom::tidy(fit_full, conf.int = TRUE)

cat("\nSummary of p-values across all terms:\n")
p_summary <- tidy_res |>
  group_by(term) |>
  summarize(
    min_p = min(p.value),
    max_stat = max(abs(statistic)),
    significant_levels = sum(p.value < 0.05)
  ) |>
  arrange(min_p)

print(p_summary, n = 30)

# Save pre-calculated cleaned data & model
saveRDS(df_clean, "data/gss_cleaned.rds")
saveRDS(m_data, "data/gss_model_data.rds")
saveRDS(fit_full, "data/gss_multinomial_model.rds")
cat("\nSaved gss_cleaned.rds, gss_model_data.rds, gss_multinomial_model.rds!\n")
